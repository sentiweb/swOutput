...
.output.open = function(filename, titre) {
 ...
 if( is.null(css) ) {
    ...
    css = file(share.path(paste0('lib/output/',theme,'.css')))
    do.copy = T
...
if(R.version$major == 2 && compareVersion(R.version$minor,"8.1") < 1) {
  #'@noRd
  safe.cat <- function(..., file="", sep=" ", append=F) {
    x = paste(..., sep=sep, collapse=sep)
    n = nchar(x)
    if(n > 10000) {
      step = 9999
      i = 1
      while(i < n) {
        last = min(i + step, n)
        cat("safe.cat:", i, " - ", last,"\n")
        r = substring(x, i, last)
        #cat(r, file=file, append=append)
        i = i + step + 1
      }
...
xprint.character <- function(x, title="", class="character", ...) {
...
#' @method xprint default
xprint.default <-  function(x, title="", ...) {
  if(isTRUE(share.option('output')$debug)) {
   cat("xprint.default\n")
  }
...
xprint.matrix=function(x, title="",...) {
  xprint.data.frame(x, title=title,...)
}

#' @method xprint table
xprint.table = function(x, title="", ...) {
  if(isTRUE(share.option('output')$debug)) {
   cat("xprint.table\n")
  }
  d = dim(x)
...
xtitle = function(..., level=NULL) {
 if( is.null(level) ) {
    level = share.option('output')$default.level
 }
...
#'  xbloc("my title")
#'  xbloc(end=T)
xbloc = function(title=NULL, end=F, style=NULL) {
 o = share.option('output')
 block.level = o$block.level
 if(end) {
  xhtml('</div></div>')
 ...
 o$block.level = block.level
 share.option('output'=o)
}
